...
const FolderChat = {
  ...
  async saveFolderChat(
    ...
    const res = await pool.query(
      ...
      INSERT INTO folder_chats
        (id, user_id, folder_name, question, answer, summarized_file_ids, session_id,
         used_secret_prompt, prompt_label, secret_id, created_at)
      VALUES
        ($1::uuid, $2, $3, $4, $5, $6::uuid[], $7::uuid, $8, $9, $10::uuid, NOW())
  ...
  async getFolderChatHistory(userId, folderName, sessionId = null) {
    let query = `
      SELECT id, user_id, folder_name, question, answer, session_id, summarized_file_ids,
             used_secret_prompt, prompt_label, secret_id, created_at
      FROM folder_chats
