...
exports.analyzeDocument = async (req, res) => {
  try {
    ...
    const tokensReserved = await TokenUsageService.checkAndReserveTokens(
      req.user.id,
      analysisCost
    );
    if (!tokensReserved) {
      return res
        .status(403)
        .json({
          message: "User token limit is exceeded for document analysis.",
        });
    }
    ...
    try {
      ...
      await TokenUsageService.commitTokens(
        req.user.id,
        analysisCost,
        `Document analysis for file ${file_id}`
      );
    } catch (aiError) {
  ...
  try {
    ...
    if (!tokensReserved) {
      return res
        .status(403)
        .json({
          message: "User token limit is exceeded for summary generation.",
        });
    }
    ...
    try {
      ...
      await TokenUsageService.commitTokens(
        userId,
        summaryCost,
        `Summary generation for file ${file_id}`
      );
    } catch (aiError) {
...
//       return res
//         .status(400)
//         .json({ error: "Document has no readable content." });
//     }
...
//         userId,
//         chatCost,
//         `AI chat for document ${file_id} (no context)`
//       );
...
//     if (chatCost && userId) {
//       await TokenUsageService.rollbackTokens(
//         userId,
//         chatCost,
//         "Token rollback due to error"
//       );
//     }
//     return res
//       .status(500)
...
//     // 🔹 Load chat history
//     const chatHistory = await FileChat.getChatHistory(file_id);
//     const formattedHistory = chatHistory
//       .map(chat => `Q: ${chat.question}\nA: ${chat.answer}`)
//       .join("\n\n");

//     // 🔹 Get chunks as context
//     const allChunks = await FileChunkModel.getChunksByFileId(file_id);
//     const documentFullText = allChunks.map(c => c.content).join("\n\n");

//     if (!documentFullText.trim()) {
//       return res.status(400).json({ error: "Document has no readable content." });
//     }

//     // 🔹 Estimate token cost
//     const chatContentLength = question.length + documentFullText.length + formattedHistory.length;
//     chatCost = Math.ceil(chatContentLength / 100);
...
//     const relevantChunkContents = relevantChunks.map(c => c.content);
...
//     // 🔹 Ask Gemini
//     const answer = await askGemini(contextPrompt, question);
...
//       history: [...chatHistory, { question: storedQuestion, answer }]
//     });
//   } catch (error) {
//     console.error("❌ Error chatting with document:", error);
//     if (chatCost && userId) {
